...
import React, { useState, useRef, useEffect } from 'react';
import { useAuth, api } from '../context/AuthContext'; // IMPORT 'api' HERE
import toast from 'react-hot-toast';
...
// ... ScoreRing and ResultPanel stay exactly the same ...
function ScoreRing({ score, size=80, color='var(--accent)' }) {
  const r = (size - 12) / 2; const circ = 2 * Math.PI * r;
  const offset = circ - (score / 100) * circ;
  return (
...
export default function Agents() {
  ...
  const [running, setRunning] = useState(false);
  const [chatOpen, setChatOpen] = useState(false);
  const [messages, setMessages] = useState([{ role:'assistant', content:'👋 Hi! I\'m PlaCIQ AI, powered by Claude (Anthropic). Ask me anything about placements, DSA, system design, resume tips, or career strategy.' }]);
  const [chatInput, setChatInput] = useState('');
  const [chatLoading, setChatLoading] = useState(false);
  const chatEndRef = useRef(null);
  ...
  const profile = {
    ...
    role: user?.profile?.targetRole || 'Software Engineer',
    companies: user?.profile?.targetCompanies || [],
    cgpa: user?.profile?.cgpa || 8.0,
    branch: user?.profile?.branch || 'CSE',
    year: user?.profile?.year || '3rd Year',
    resumeText: user?.profile?.resumeText || '',
  ...
  // FIX 1: Using 'api' instance from context
  const runAgent = async agentId => {
    setStatuses(s => ({ ...s, [agentId]: S.running }));
    setSelected(agentId);
    try {
      // Changed from axios.post(`${API}/...`) to api.post('/...')
  ...
  const runAll = async () => {
    ...
    setRunning(false);
    toast.success('🎉 All 5 agents complete!');
  };

  // FIX 2: Using 'api' instance for Chat
  const sendChat = async e => {
    e?.preventDefault();
    if (!chatInput.trim()) return;
    const msg = chatInput; setChatInput('');
    ...
    try {
      ...
      setMessages(m => [...m, { role:'assistant', content:'AI unavailable. Check connection.' }]);
    } finally {
      setChatLoading(false);
      setTimeout(() => chatEndRef.current?.scrollIntoView({ behavior:'smooth' }), 100);
    }
  ...
  const allDone = AGENTS.every(a => statuses[a.id] === S.done);
  const QUICK_PROMPTS = ['Crack Google interviews?', 'DSA strategy', 'Target companies?', 'Resume tips?'];
    ...
    <div style={{ padding:'28px 32px', maxWidth:1200 }}>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:0, padding:'18px 24px', background:'var(--surface)', border:'1px solid var(--border)', borderRadius:16, marginBottom:24, overflowX:'auto' }}>
        {AGENTS.map((a, i) => {
          ...
          return (
            <React.Fragment key={a.id}>
              <div className="agent-node" onClick={() => !running && runAgent(a.id)} style={{ opacity: running && st !== S.running ? 0.5 : 1 }}>
                <div className="agent-circle" style={{
                  background: st === S.done ? a.color : st === S.running ? `${a.color}30` : 'var(--surface2)',
                  borderColor: st ? a.color : 'var(--border2)',
                  boxShadow: st === S.running ? `0 0 16px ${a.color}60` : st === S.done ? `0 0 12px ${a.color}40` : 'none',
      ...
      <div style={{ display:'grid', gridTemplateColumns: chatOpen ? '1fr 380px' : '1fr', gap:20 }}>
        ...
        {chatOpen && (
          <div style={{ display:'flex', flexDirection:'column', height:'calc(100vh - 160px)', background:'var(--surface)', border:'1px solid var(--border)', borderRadius:18, overflow:'hidden', position:'sticky', top:20 }}>
            ...
            <form onSubmit={sendChat} style={{ padding:'12px', borderTop:'1px solid var(--border)' }}>
              <div style={{ display:'flex', gap:8 }}>
                <input value={chatInput} onChange={e => setChatInput(e.target.value)} placeholder="Ask anything..." style={{ flex:1, padding:'10px' }} />
                <button type="submit" disabled={chatLoading} className="btn btn-primary">↑</button>
              </div>
            </form>
          </div>
        )}
